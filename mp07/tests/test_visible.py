...
class TestStep(unittest.TestCase):
    ...
    def test_unify(self):
        ...
        self.assertEqual(tuple(tsubs.items()), tuple(subs.items()),
    ...
    def test_apply(self):
        ...
        asets = [ tuple(tuple(x) for x in applications[i]['antecedents']) for i in range(2) ]
        self.assertIn(tuple(tuple(x) for x in tapps[0]['antecedents']),asets,
                      '''applications should contain a rule with antecedents %s, 
                      but does not'''%(tapps[0]['antecedents']))
        self.assertIn(tuple(tuple(x) for x in tapps[1]['antecedents']),asets,
                      '''applications should contain a rule with antecedents %s, 
